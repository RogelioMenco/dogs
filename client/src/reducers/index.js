  ...
  switch (action.type) {
      ...
      return {
        ...
        breedsToFilter: state.breedsToFilter.sort((a, b) => {
          ...
          if (a.weight.split(' - ').length === 2) {
            ...
            x = parseInt(a.weight.split(' - ')[0]).toString();
          }
          if (b.weight.split(' - ').length === 2) {
            ...
            y = parseInt(b.weight.split(' - ')[0]).toString();
          }
          return x - y;
        }),
      };
    ...
    case SEARCH_BREED:
      if (action.payload === 'ALL') {
        ...
        if (state.breedsToFilter[0] !== false) {
          searchedBreed = state.breedsToFilter.filter((breed) =>
            breed.name.includes(action.payload),
          );
        }
    ...
    case FILTER_BY_ORIGIN:
      if (action.payload === 'ALL') {
        ...
        let breedsByOrigin = [];
        breedsByOrigin = state.breeds.filter(
          (breed) => breed.origin === action.payload,
        );
